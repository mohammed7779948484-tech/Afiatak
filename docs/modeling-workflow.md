...
5. Create a view that selects canonical IDs without copying descriptions or rules.
6. Run `validate-view`, `traceability`, and `render`.
7. Run `qa`; inspect the preview when draw.io is available. Correct semantics/views/layout sources, not only generated XML.
