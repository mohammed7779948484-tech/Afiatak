...
def export_deployment_drawio(model, view, output: Path) -> None:
    ...
    for item_id, node_layout in layout.nodes.items():
        ...
        _geometry(name, node_layout.title_bounds.x - box.x, node_layout.title_bounds.y - box.y, node_layout.title_bounds.width, node_layout.title_bounds.height)
        for contained_label, contained_bounds in node_layout.contained:
            contained = _cell(cells, next_id("contained"), contained_label, style=f"shape=module;html=1;align=center;verticalAlign=middle;fontSize={int(TOKENS.contained_font_size)};fontColor=#242424;fontFamily=Arial;fillColor=#FFFFFF;strokeColor=#343434;strokeWidth={TOKENS.contained_stroke_width};whiteSpace=wrap;", vertex="1", parent=node_id)
            _geometry(contained, contained_bounds.x - box.x, contained_bounds.y - box.y, contained_bounds.width, contained_bounds.height)
