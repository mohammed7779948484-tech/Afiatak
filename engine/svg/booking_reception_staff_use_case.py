...
def render_booking_reception_staff_svg(model: SemanticModel, view: ViewSpec, output: Path, composition, description: str) -> Path:
    ...
    p = composition.PACKAGE; group = ET.SubElement(root, _tag("g"), {"aria-label": "Booking & Reception Staff Package UML container"})
    path = f"M {p.x:g} {p.y+24:g} H {p.x+220:g} V {p.y:g} H {p.x+p.width:g} V {p.y+p.height:g} H {p.x:g} Z"
    ET.SubElement(group, _tag("path"), {"id": "booking-reception-staff-package", "d": path, "class": "package", **_bounds_attributes(p)})
    ET.SubElement(group, _tag("rect"), {"x": str(p.x), "y": str(p.y), "width": "220", "height": "34", "class": "package-tab"})
    _text(group, view.options.get("packageName", "Booking & Reception Staff Package"), p.x+110, p.y+22, "boundary-title", limit=34)
    ...
    nodes = ET.SubElement(root, _tag("g"), {"aria-label": "Booking & Reception Staff use cases"})
