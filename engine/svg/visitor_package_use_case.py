...
from engine.compositions.aafiatak_visitor_package_use_case import (
    ACTORS,
    BOUNDARY,
    CANVAS,
    CASE_ROLES,
...
def render_visitor_package_use_case_svg(model: SemanticModel, view: ViewSpec, output: Path) -> Path:
    ...
    if set(view.include) != expected:
        raise ValueError("composition/view element mismatch")
    if set(view.relations) != set(ROUTES):
        raise ValueError("composition/view relation mismatch")
    ...
    description.text = "Detailed UML Visitor Package Use Case Diagram for the Aafiatak medical appointment booking system."
    ...
    package_group = ET.SubElement(root, _tag("g"), {"aria-label": "Visitor Package UML container"})
    ...
    ET.SubElement(package_group, _tag("path"), {"id": "visitor-package", "d": package_path, "class": "package", **_bounds_attributes(PACKAGE)})
    ET.SubElement(package_group, _tag("rect"), {"x": str(PACKAGE.x), "y": str(PACKAGE.y), "width": "220", "height": "34", "class": "package-tab"})
    _text(package_group, view.options.get("packageName", "Visitor Package"), PACKAGE.x + 110, PACKAGE.y + 22, "boundary-title", limit=32)

    field_group = ET.SubElement(root, _tag("g"), {"aria-label": "Presentation-only Visitor Package neighborhoods"})
    accent_by_role = {"access": theme["colors"]["access"], "patient": theme["colors"]["patient"]}
    ...
    for relation_id in view.relations:
        ...
        ET.SubElement(relation_group, _tag("path"), {"id": relation_id.replace(".", "-"), "data-relation-id": relation_id, "data-source": relation.source, "data-target": relation.target, "data-points": _points_data(ROUTES[relation_id]), "d": _path_data(ROUTES[relation_id]), "class": "dependency" if dependency else "association"})
        if dependency:
            stereotype_position, _ = DEPENDENCY_LABELS[relation_id]
            _text(relation_group, f"<<{relation.type}>>", *stereotype_position, "stereotype", limit=30)

    node_group = ET.SubElement(root, _tag("g"), {"aria-label": "Visitor Package use cases"})
